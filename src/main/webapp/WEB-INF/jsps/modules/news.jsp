...
if(rs.first())  {
		...
		<!-- newsid = <%= news_newsid %> -->
		    ...
		    <td class="mainModuleItemTitle">
		      ...
		      <span style="float:right;position:relative">
				...
				if(news_uname.equals(logged_uname) || logged_position.equals("Administrator"))  {
					%>
					<a href="<%= response.encodeURL("?nav=User&view=e_news&newsid=" + news_newsid) %>">edit</a> <a href="<%= response.encodeURL("?nav=Home&action=f_dnews&newsid=" + news_newsid) %>">delete</a>
					<%
				}
				%>
		      </span>
		  ...
		  <tr>
		    <td class="mainModuleItemSubTitle">
		      <span style="float:left;position:relative">Posted <%= news_mdate %>/<%= news_ddate %>/<%= news_ydate %> - <%= news_hdate %>:<%= news_mindate %> by <a href="<%= response.encodeURL("?nav=Members&view=member&member_uname=" + news_uname) %>"><%= news_uname %></a> in <a href="<%= response.encodeURL("?category=" + rs.getString("cname")) %>"><%= rs.getString("cname") %></a></span>
		      <span style="float:right;position:relative"><a href="<%= response.encodeURL("?view=news_post&news_id=" + news_newsid) %>#comments"><%= comment_title %>s(<%= news_comments %>)</a></span>
		    </td>
		  </tr>
		</table>
		<br>
	...
	if(!news_vars.equals(""))  {
		rs = stmt.executeQuery("Select count(newsid) as c from " + dbPrefix + "news as news where news.varsname = '" + siteName + "' and newsid in (select newsid FROM " + dbPrefix + "news AS n WHERE EXTRACT(MONTH FROM n.postdate) = " + news_month + " AND EXTRACT(YEAR FROM n.postdate) = " + news_year + " and n.varsname = '" + siteName + "' ORDER BY n.postdate DESC limit " + inc + " offset " + (start + inc) + ")");
	}
	else if(!news_cat.equals(""))  {
		rs = stmt.executeQuery("Select count(newsid) as c from " + dbPrefix + "news as news where news.varsname = '" + siteName + "' and newsid in (select newsid FROM " + dbPrefix + "news AS n WHERE cname = '" + news_cat + "' and n.varsname = '" + siteName + "' ORDER BY n.postdate DESC limit " + inc + " offset " + (start + inc) + ")");
	}
	else  {
		rs = stmt.executeQuery("Select count(newsid) as c from " + dbPrefix + "news as news where news.varsname = '" + siteName + "' and newsid in (select newsid FROM " + dbPrefix + "news AS n where n.varsname = '" + siteName + "' ORDER BY n.postdate DESC limit " + inc + " offset " + (start + inc) + ")");
	}
	...
	if(rs.getInt("c") > 0)  {
		%>
		<a href="<%= response.encodeURL("?" + news_vars + "&category=" + news_cat + "&start=" + (start + inc)) %>"> Previous </a>
		<%
	}

	if(!news_vars.equals(""))
		rs = stmt.executeQuery("Select count(newsid) as c from " + dbPrefix + "news as news where news.varsname = '" + siteName + "' and newsid in (select newsid FROM " + dbPrefix + "news AS n WHERE EXTRACT(MONTH FROM n.postdate) = " + news_month + " AND EXTRACT(YEAR FROM n.postdate) = " + news_year + " and n.varsname = '" + siteName + "' ORDER BY n.postdate DESC)");
	else if(!news_cat.equals(""))
		rs = stmt.executeQuery("Select count(newsid) as c from " + dbPrefix + "news as news where news.varsname = '" + siteName + "' and newsid in (select newsid FROM " + dbPrefix + "news AS n WHERE cname = '" + news_cat + "' and n.varsname = '" + siteName + "' ORDER BY n.postdate DESC)");
	else
		rs = stmt.executeQuery("Select count(newsid) as c from " + dbPrefix + "news as news where news.varsname = '" + siteName + "' and newsid in (select newsid FROM " + dbPrefix + "news AS n where n.varsname = '" + siteName + "' ORDER BY n.postdate DESC)");
	...
	if(start - inc >= 0)  {
		%>
		<span style="float:right;position:relative"><a href="<%= response.encodeURL("?" + news_vars + "&category=" + news_cat + "&start=" + (start - inc)) %>"> Next </a></span>
		<%
	}
}
else  {
