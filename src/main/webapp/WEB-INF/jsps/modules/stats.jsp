	...
	<tr>
	  <td class="sideModuleContent">
	    <%
		try  {
		rs = stmt.executeQuery("SELECT COUNT(distinct cname) FROM " + dbPrefix + "news where varsname = '" + siteName + "'");
		if(rs.first())  {
		...
		rs = stmt.executeQuery("SELECT COUNT(newsid) FROM " + dbPrefix + "news where varsname = '" + siteName + "'");
		if(rs.first())  {
			%> <span style="float:left;position:relative">Posts:</span> <span style="float:right;position:relative"><%= rs.getString("count") %></span><br> <%
		}
		rs = stmt.executeQuery("SELECT COUNT(comid) FROM " + dbPrefix + "comment where varsname = '" + siteName + "'");
		if(rs.first())  {
		...
		rs = stmt.executeQuery("SELECT COUNT(uname) FROM " + dbPrefix + "member natural join " + dbPrefix + "member_site where varsname = '" + siteName + "'");
		if(rs.first())  {
		...
		rs = stmt.executeQuery("SELECT COUNT(guestid) FROM " + dbPrefix + "guestbook where varsname = '" + siteName + "'");
		if(rs.first())  {
		...
		rs = stmt.executeQuery("SELECT guestid, name, EXTRACT(DAY FROM dsign) as d, EXTRACT(MONTH FROM dsign) as m, EXTRACT(YEAR FROM dsign) as y FROM " + dbPrefix + "guestbook where varsname = '" + siteName + "' order by dsign desc limit 1");
		if(rs.first())  {
			%>
		...
		rs = stmt.executeQuery("Select uname, EXTRACT(DAY FROM regdate) as d, EXTRACT(MONTH FROM regdate) as m, EXTRACT(YEAR FROM regdate) as y FROM " + dbPrefix + "member where varsname = '" + siteName + "' order by regdate desc limit 1");
		if(rs.first())  {
			%>
		...
		*/
		...
		rs = stmt.executeQuery("select stats_count, stats_total_ms, EXTRACT(YEAR FROM stats_date) as year, EXTRACT(MONTH FROM stats_date) as month, EXTRACT(DAY FROM stats_date) as d from " + dbPrefix + "site_vars where varsname = '" + siteName + "'");
